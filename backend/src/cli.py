...
def cmd_health(_args: argparse.Namespace, _payload: dict[str, Any]) -> int:
...
def cmd_projects_list(_args: argparse.Namespace, _payload: dict[str, Any]) -> int:
    return _ok(projects.list_projects())


def cmd_projects_get(args: argparse.Namespace, _payload: dict[str, Any]) -> int:
    return _ok(projects.read_files(args.name))


def cmd_projects_register(_args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(projects.register(payload))


def cmd_projects_create(_args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(projects.create(payload))


def cmd_projects_update(args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(projects.update_compose(args.name, payload))


def cmd_projects_unregister(args: argparse.Namespace, payload: dict[str, Any]) -> int:
...
def cmd_projects_scan(_args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(projects.scan(max_depth=int(payload.get("max_depth") or 4)))


def cmd_projects_action(args: argparse.Namespace, payload: dict[str, Any]) -> int:
...
def cmd_certs_list(_args: argparse.Namespace, _payload: dict[str, Any]) -> int:
    return _ok(certs.list_certs())


def cmd_certs_get(args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(certs.get_cert(args.name, include_pem=bool(payload.get("include_pem"))))


def cmd_certs_import(_args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(certs.import_cert(payload))


def cmd_certs_generate(_args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(certs.generate(payload))


def cmd_certs_delete(args: argparse.Namespace, _payload: dict[str, Any]) -> int:
    return _ok(certs.delete(args.name))


def cmd_certs_assign(args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return _ok(
        certs.assign(
            args.name,
...
def cmd_logs_tail(args: argparse.Namespace, payload: dict[str, Any]) -> int:
    text = logs.tail(
        args.name,
        service=args.service or payload.get("service"),
...
def cmd_logs_follow(args: argparse.Namespace, payload: dict[str, Any]) -> int:
    return logs.follow(
        args.name,
        service=args.service or payload.get("service"),
        lines=int(args.lines or payload.get("lines") or 200),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if handler is None:
        return _fail(f"未知命令: {args.command}")
    try:
